# Social Content Generation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Generate 3-4 daily Instagram & TikTok copywriting captions for @kapster.myid/@kapster.my.id with trend analysis and Telegram-based review.

**Architecture:** Cron-triggered script using Groq LLM for trend research + copy generation, saving to `social_posts` table, delivering to Telegram for user review. No social media API integration — posting is manual.

**Tech Stack:** Groq API (`llama-3.3-70b-versatile`), Supabase (PostgreSQL + admin client), Telegram Bot API, Next.js API route

---

### Task 1: Create `social_posts` table migration

**Files:**
- Create: `supabase/migrations/add_social_posts.sql`
...
Expected: `social_posts` table created

- [ ] **Step 3: Generate TypeScript types**

Run: `npx supabase gen types typescript --linked > lib/supabase/types.ts`
Expected: types updated with `Database["public"]["Tables"]["social_posts"]`

- [ ] **Step 4: Commit**

```bash
git add supabase/migrations/add_social_posts.sql lib/supabase/types.ts
git commit -m "feat: add social_posts table for social media content generation"
```

---

### Task 2: Add cron job migration for social content

**Files:**
- Create: `supabase/migrations/add_social_cron.sql`
...
```bash
git add supabase/migrations/add_social_cron.sql
git commit -m "feat: add daily cron job for social content generation"
```

---

### Task 3: Create API route for cron trigger

**Files:**
- Create: `app/api/cron/generate-social-content/route.ts`
...
```bash
git add app/api/cron/generate-social-content/route.ts
git commit -m "feat: add API route for social content cron trigger"
```

---

### Task 4: Write main generation script — `scripts/generate-social-content.ts`

**Files:**
- Create: `scripts/generate-social-content.ts`
...
Pastikan variasi pillar sesuai persentase di atas untuk 5 topik.`;
  ...
  // Phase 2: Content Selection — pick 3-4 topik with pillar variety
  console.log("[social-gen] Phase 2: Content Selection...");
  const selectedTopics = trendData.topics.slice(0, 4);
  console.log(`[social-gen] Selected ${selectedTopics.length} topics for generation`);
...
{"caption": "caption lengkap", "hashtags": ["#tag1", "#tag2"], "hook_type": "question|fact|pain_point|statistic|story"}`;
    ...
    try {
      ...
      contents.push({
        platform: topic.platform_hint as SocialContentItem["platform"],
        caption: copyData.caption,
        hashtags: copyData.hashtags || [],
        content_type: topic.pillar as SocialContentItem["content_type"],
...
🔍 <b>Tren:</b> ${item.trend_insight}`;
  ...
  for (const item of contents) {
    const { error } = await supabase.from("social_posts").insert({
      ...
      trend_analysis: { hook_type: item.hook_type, trend_insight: item.trend_insight },
...
git commit -m "feat: add social content generation script"
```

---

### Task 5: Deploy to server and test
...
```bash
curl -X POST https://kapster.my.id/api/cron/generate-social-content \
  -H "Authorization: Bearer kpBlogGen2026!" \
  -H "Content-Type: application/json" \
  -d '{}'
```

Expected: `{"ok":true,"message":"Social content generation started"}`

- [ ] **Step 5: Verify Telegram delivery**

Check Telegram chat for generated content previews. 3-4 messages with platform, pillar, caption, hashtags, and trend insight.
