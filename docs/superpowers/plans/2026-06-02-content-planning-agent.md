# Content Planning Agent Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Split content generation into two agents — research agent (planning) + content agent (execution) — with measurable quality tracking.

**Architecture:** Single script `generate-social-content.ts` with `--mode` flag (`research` | `generate`). Research mode runs Trend-Pulse → LLM analysis → saves to new `content_plans` table. Generate mode reads pending plans → generates copy → saves to `social_posts`. New tables `content_plans` and `content_metrics` for tracking.

**Tech Stack:** Groq (content gen), OpenRouter (QA), Trend-Pulse (research), Supabase PostgreSQL

---

### Task 1: Task 1: Database migration — create new tables + column

**Files:**
- Execute: direct psql to Supabase
...
- [ ] **Step 2: Add content_plan_id to social_posts**

```sql
ALTER TABLE social_posts ADD COLUMN IF NOT EXISTS content_plan_id UUID REFERENCES content_plans(id);
```

- [ ] **Step 3: Create content_metrics table**
...
```bash
psql "$DATABASE_CONNECTION_STRING" -f /tmp/migration.sql
```

- [ ] **Step 5: Commit**

```bash
git add docs/superpowers/plans/2026-06-02-content-planning-agent.md
git commit -m "docs: add implementation plan for content planning agent"
```

---

### Task 2: Task 2: Refactor generate-social-content.ts — add research mode + Agent Konten mode

**Files:**
- Modify: `scripts/generate-social-content.ts`

- [ ] **Step 1: Restructure script — add --mode flag + Agent Konten logic**
...
async function main() {
  ...
  // If mode=research, run research pipeline
  if (modeArg === "research") {
  ...
  // Get brief — either from user topic or from pending plan
  let brief: string;
  ...
  if (USER_TOPIC) {
    ...
    if (!pendingPlan) {
      console.log("[social-gen] No pending plans and no user topic, exiting.");
      // Run research on-demand
      console.log("[social-gen] Running research on-demand...");
      await runResearch(supabase);
  ...
  // Phase 1: Trend Research (only for manual mode without pending plan — Trend-Pulse not needed since brief already exists)
  let trendInsight = brief;
  if (USER_TOPIC) {
    // Trend-Pulse optional — get trending context to enrich brief
    // For now, keep brief as-is
  }
...
{"title": "hook spesifik untuk card", "description": "satu baris max 100 char", "caption": "caption lengkap", "hashtags": ["#tag1", "#tag2"], "content_type": "educational|solution"}`;
  ...
  try {
    ...
    if (isDuplicate(item.topic)) {
      ...
      // Mark plan as used but with note
      if (planId) {
    ...
    if (review.score < 4) await recordMetric(supabase, "qa_regen_rate", 1, {});

    if (review.score >= 4) {
      await recordMetric(supabase, "qa_regen_rate", 0, {});
    }

    // Card + Telegram (same as before)
    console.log("[social-gen] Phase 5: Generating card images...");
    try {
      const hook = item.card_title;
      const description = item.card_description || "";
...
// --- Agent Riset (inline, same as content-research.ts) ---
...
async function recordMetric(supabase: any, metricName: string, value: number, metadata?: Record<string, unknown>) {
  const today = new Date().toISOString().split("T")[0];
  await supabase.from("content_metrics").upsert(
    { metric_date: today, metric_name: metricName, metric_value: value, metadata: metadata || {} },
...
- [ ] **Step 2: Test generate mode with user topic**

Run: `set -a && source .env && set +a && npx tsx scripts/generate-social-content.ts --platform=ig --topic="Test: tips manajemen staf barbershop"`

Expected: Generates content, saves to social_posts with content_plan_id = null

- [ ] **Step 3: Test generate mode with pending plan**

Run research first: `npx tsx scripts/generate-social-content.ts --mode=research`

Then generate: `npx tsx scripts/generate-social-content.ts --platform=ig`

Expected: Takes pending plan, generates content, marks plan as used

- [ ] **Step 4: Commit**

```bash
git add scripts/generate-social-content.ts
git commit -m "feat: add research mode + content planning agent pipeline to generate script"
```

---

### Task 4: Task 3: New API endpoints and webhook update

**Files:**
- Create: `app/api/cron/research/route.ts`
- Modify: `app/api/cron/generate-social-content/route.ts`
- Modify: `app/api/telegram/webhook/route.ts`

- [ ] **Step 1: Create research cron endpoint**
...
- [ ] **Step 2: Update generate-social-content cron endpoint**

```typescript
// app/api/cron/generate-social-content/route.ts
import { NextResponse } from "next/server";
import { execFile } from "child_process";
...
- [ ] **Step 3: Update webhook to use same script with --platform, --topic**

```typescript
// In app/api/telegram/webhook/route.ts — update "/konten" handler
// Change from: execFile("npx", ["tsx", scriptPath, `--platform=${platform}`, `--topic=${topic}`], ...)
// To: execFile("npx", ["tsx", scriptPath, `--platform=${platform}`, `--topic=${topic}`], ...)
// (the --mode=generate is implicit default, so no change needed)
```

- [ ] **Step 4: Commit**

```bash
git add app/api/cron/research/route.ts app/api/cron/generate-social-content/route.ts app/api/telegram/webhook/route.ts
git commit -m "feat: add research cron endpoint, update generate cron and webhook"
```

---

### Task 4: Remove dead code — hardcoded fallback, old curation template

**Files:**
- Verify: `scripts/generate-social-content.ts` — sudah tidak ada hardcoded fallback topics, JSON template untuk riset

- [ ] **Step 1: Search for any remaining dead code**

```bash
grep -n "Tips mengelola antrian barbershop di jam sibuk\|Cara hitung komisi kapster tanpa ribet\|USER_TOPIC.*fallback\|hardcoded" scripts/generate-social-content.ts
```

Expected: no matches

- [ ] **Step 2: Verify cleanup commit**

```bash
git add scripts/generate-social-content.ts
git commit -m "chore: remove hardcoded fallback topics and stale curation template"
```

---

### Task 5: Add quality monitoring — alert on degradation

**Files:**
- Modify: `scripts/generate-social-content.ts`

- [ ] **Step 1: Add monitoring function and call it at end of main()**

Add at the end of `scripts/generate-social-content.ts`:

```typescript
async function checkQualityAlerts(supabase: any) {
  // Check template_ratio for last 7 days
  ...
  // Check qa_avg_score for last 3 days
...
Add to the end of `main()` in generate mode:
```typescript
if (modeArg !== "research") {
...
```bash
git add scripts/generate-social-content.ts
git commit -m "feat: add quality monitoring and Telegram alerts for content degradation"
```
...
- [ ] **Step 3: Verify research endpoint**

```bash
curl https://kapster.my.id/api/cron/research
```

Expected: `{"ok":true,"log":["[research] Starting...",...,"[research] Plan saved"]}`

- [ ] **Step 4: Verify database tables**

```bash
psql "$DATABASE_CONNECTION_STRING" -c "SELECT metric_name, metric_value FROM content_metrics ORDER BY created_at DESC LIMIT 5;"
```

Expected: Shows research_success = 1, plans_created = 1

- [ ] **Step 5: Test manual /konten**

Send `/konten ig` from Telegram

Expected: Takes pending plan, generates content, sends to Telegram

- [ ] **Step 6: Verify metrics are recording**

```bash
psql "$DATABASE_CONNECTION_STRING" -c "SELECT * FROM content_metrics ORDER BY created_at DESC;"
```

Expected: Shows posts_created, qa_avg_score, etc.
